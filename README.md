...
The schema is split one file per table (`01_admin.sql` ... `21_triggers.sql`)
under `backend/schema/`, in dependency order so foreign keys always find the
table they reference already created — plus dedicated files for views,
stored procedures, and triggers. `run_all.sql` sources them all in the
...
This is a one-time step — you don't need to redo it every time you start
the servers, and it isn't safe to re-run against a database that already
has these tables (you'll get "table already exists" errors). If you need
a clean slate, drop the database first.
...
6. As an officer or admin, try **Deposit Requests** — record a walk-in cash
   deposit, then approve it against a real account number to see the
   account holder's balance update and a notification land in their account
...
- Registration now collects **Email** (backend requires it) and only offers
  **Bank User** signup — Officer accounts are created by an Admin via the
...
- **Manage Users** now has a working remove button, backed by a real
  delete-user endpoint that cleans up the user's accounts, transactions, and
...
- New **Deposit Requests** page (officer + admin) for recording and
  approving walk-in cash deposits — the account holder gets a real
